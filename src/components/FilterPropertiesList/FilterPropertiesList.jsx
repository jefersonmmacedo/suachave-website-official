...
export function FilterPropertiesList({status, typeProperty, subTypeProperty, city, uf, quarto, banheiro, suítes, garagem, petsProperty, furnishedProperty}) {
    console.log({status, city, uf, quarto, banheiro, suítes, garagem, petsProperty, furnishedProperty})
    ...
    const [citySelected, setCitySelected] = useState(city !== null ? `${city} - ${uf}` : "");
    ...
    function handleNewSearchProperty(e) {
        e.preventDefault();
        window.open(`/imoveis/${statusProperty}?city=${cityNew}&uf=${ufNew}&tipo=${type}&subtipo=${subType}&quartos=${bedroom}&suites=${suite}&banheiros=${restroom}&garagem=${garage}`,"_self");
    }
